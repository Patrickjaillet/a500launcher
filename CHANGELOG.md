...
### Added
- XInput gamepad auto-detection: when Port 1 is set to Joystick and a controller
  is connected, the configuration uses the physical device (`joy0`) instead of the
  keyboard-emulated joystick (`joy1`).
- Help window (new "Help" menu entry) listing the emulator and launcher keyboard
  shortcuts, including how to quit WinUAE and return to the launcher.
